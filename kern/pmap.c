...
page_init(void)
{
	...
	int io = (EXTPHYSMEM - IOPHYSMEM) / PGSIZE;
	size_t i;
	for (i = 0; i < npages; i++) {
	       if(i == 0){
			pages[i].pp_ref = 1;
	        ...
	        else if(i >= npages_basemem && i < npages_basemem + io + num_alloc){
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
			continue;
    		}
...
{	
	...
	if(!(*pg_entry & PTE_P)){
		...
		new_page=page_alloc(ALLOC_ZERO);
			...
			new_page->pp_ref++;
			*pg_entry = (page2pa(new_page) | PTE_P |PTE_W | PTE_U);
		}
	}

	
	pg_table = KADDR(PTE_ADDR(*pg_entry));
